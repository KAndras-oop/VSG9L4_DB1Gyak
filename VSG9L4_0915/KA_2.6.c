...
int main(){
    ...
    if(!fp){
        ...
        fclose(fp);
        printf("letrehoztam mert nem volt\n");
        exit(0);
    }
    ...
    while(i<szam){
        ...
        fwrite(&(Auto[i]), sizeof(struct jarmu),1,fp);
        fwrite("\n", 1, 1, fp);
...
int piros(){
    ...
    char ch;
    int pszin = 0;
    fseek(fp, (ossz-1)*sizeof(struct jarmu),0);
    fread(&(Auto[ossz]), sizeof(struct jarmu), 1,fp);
    ...
    for(int i=0; i<ossz; i++)
    {
            ...
            fread(&(Auto[i]), sizeof(struct jarmu), 1, fp);
